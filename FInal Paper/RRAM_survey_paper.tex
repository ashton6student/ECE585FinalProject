...
Three technologies---STT-MRAM, RRAM, and PCM---have emerged as the serious contenders for embedded NVM at advanced nodes, with each occupying a different niche~\cite{ref_hellenbrand2024}. STT-MRAM is the most mature at FinFET-class nodes, with shipping eMRAM macros at TSMC 22ULL/16~nm, Samsung 28FDS, and GlobalFoundries 22FDX. PCM had a major standalone deployment in Intel--Micron 3D~XPoint (Optane) from 2017--2022, and is currently shipping in embedded form at STMicroelectronics in their 28~nm FD-SOI automotive platform~\cite{ref_hellenbrand2024}. RRAM is most established at planar bulk and FD-SOI nodes, with embedded products at TSMC 40/28/22~nm and at GlobalFoundries via Weebit~Nano. RRAM is attractive because its cell is a simple two-terminal metal--insulator--metal (MIM) stack of materials already qualified in CMOS fabrication, it is built in the back-end-of-line (BEOL) above the transistors, and its mask adder is small. The cell does not stand alone; in production it is paired with a select MOSFET in a 1-transistor--1-resistor (1T1R) configuration, which makes the technology directly relevant to MOSFET device modeling.
...
\begin{itemize}
\item PCM RESET (amorphization) requires heating the chalcogenide above its melting temperature ($T_m\!\approx\!600\,^\circ$C for Ge\textsubscript{2}Sb\textsubscript{2}Te\textsubscript{5}) followed by rapid quenching, while SET (crystallization) holds the cell between the glass transition $T_g$ and $T_m$ to allow nucleation and growth. The melt-quench requirement is what drives RESET currents to ${\sim}100~\mu$A or more in scaled cells. The resulting select-FET width is several times larger than the corresponding VCM 1T1R, eroding the cell-area win~\cite{ref_hellenbrand2024,ref_wong2012}.
